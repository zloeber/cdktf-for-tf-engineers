...
After importing the (mostly) autogenerated stack for the cluster configuration via `import { Cluster1Stack } from "./cluster1";` we instantiate a new instance of it in `main.ts` (after our other stack). The synth process will generate the terraform for it in its own folder as you may expect from an entirely separate terraform project pipeline.
